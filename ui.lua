...
-- library init
	...
	writefile(library.directory .. "/fonts/fs-tahoma-8px.ttf", game:HttpGet("https://github.com/i77lhm/storage/blob/main/fonts/fs-tahoma-8px.ttf"))
	...
	writefile(library.directory .. "/fonts/dddd.ttf", http_service:JSONEncode(tahoma))

	library.font = Font.new(getcustomasset(library.directory .. "/fonts/dddd.ttf"), Enum.FontWeight.Regular)
...
-- library functions 
	...
	-- 
		...
		function get_preview_flag(flag_name, fallback)
		...
		function get_preview_color(flag_name, fallback)
	...
	-- elements 
		...
		function library:window(properties)
			...
			-- theming 
				...
				local watermark = library:watermark({default = os.date('aevryn |  - %b %d %Y - %H:%M:%S')})  

				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date('aevryn - Beta - %b %d %Y - %H:%M:%S'))
					end 
				end) 
				...
				section:button({name = "Join New Server", callback = function()
					local Req, data = game:GetService("HttpService"):JSONDecode(game:HttpGetAsync("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100")).data, nil
					table.sort(Req, function(a, b)
						 return a.ping < b.ping
					end)
					for i = 1, #Req do 
						if Req[i].playing < flags["max_players"] and Req[i].id ~= game.JobId then 
							data = Req[i]
							break
						end
					end
			...
			-- playerlist 
				...
				section:button({name = "Teleport to", callback = function()
					local target = players:FindFirstChild(flags["PLAYERLIST_TEXTBOX"])
					if target then 
		...
		function library:esp_preview(properties)
			local cfg = {items = {}, rotation = 0; objects = {}, skeleton = {}};

			local character = players:CreateHumanoidModelFromDescription(players:GetHumanoidDescriptionFromUserId(lp.UserId),Enum.HumanoidRigType.R15)

			character.Name = "ESP_Preview"
            ...
            local function world_to_viewport(cam, pos)
                local v, on_screen = cam:WorldToViewportPoint(pos)
